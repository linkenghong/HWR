...
from model import HWDB_SegNet, HWDB_GoogLeNet
...
if __name__ == "__main__":
    ...
    model = HWDB_SegNet(num_classes)
    if torch.cuda.is_available() and torch.cuda.device_count() > 1:
        model = nn.DataParallel(model)
        model = model.cuda()
    model.module.load_state_dict(torch.load('checkpoints/HWDB_SegNet_8_iter_34.pth'))

    #####################################################################################
    # 减少fc层神经元数后
    # premodel = HWDB_GoogLeNet(num_classes)
    # if torch.cuda.is_available():
    #     premodel = premodel.cuda()
    # premodel.load_state_dict(torch.load('checkpoints/HWDB_GoogLeNet_iter_825.pth',  map_location=torch.device('cuda:1')))

    # model_dict = model.state_dict()
    # pretrained_dict = {k: v for k, v in premodel.state_dict().items() if k in model_dict}
    # pretrained_dict['fc1.weight'] = premodel.state_dict()['fc1.weight'][::8]
    # pretrained_dict['fc1.bias'] = premodel.state_dict()['fc1.bias'][::8]
    # pretrained_dict['fc2.weight'] = premodel.state_dict()['fc2.weight'][:,::8]
    ...
    for epoch in range(35, epochs):
        ...
        torch.save(model.module.state_dict(), pth_path + 'HWDB_SegNet_8_iter_%d.pth' % epoch)