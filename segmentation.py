...
def find_best_borders_connect(img, name, net_num = 287, threshold = 200, direction = 0, overlap = 0.85, net_type = 'S'):
    limg = img.convert('L').point(lambda p: 0 if p > threshold else 1, '1')
    img_data = np.array(limg, dtype=np.int32)
    direction_sum = img_data.sum(axis=direction)
    up, down = np.where(direction_sum>0)[0][0], np.where(direction_sum>0)[0][-1]
        ...
        for i in range(area_num):
            for j in range(i, area_num):
                if (merge_area[j][1] - merge_area[i][0] >= char_min and
                    ...
                    # display(pred_img)
                    charM[i][j], confM[i][j], _, charConfM[i][j] = image_prediction(pred_img, topk = 1, net_num = net_num, labels = name_list, net_type = net_type)
...
# 投影高斯平滑极值法
def find_best_borders_gas(img, name, net_num = 287, threshold = 200, direction = 0, gas_std = 8, gap = 20, net_type = 'S'):
    name_list = list(name)
    char_num = len(name)
    limg = img.convert('L').point(lambda p: 0 if p > threshold else 1, '1')
    img_data = np.array(limg, dtype=np.int32)
    ...
    local_min = find_local_min(gas8, gap = gap)
        ...
        for i in range(min_num):
            for j in range(i+1, min_num):
                if(local_min[j] - local_min[i] >= char_min and local_min[j] - local_min[i] <= char_max):
                    ...
                    pred_img = img.crop(border)
                    # display(pred_img)
                    charM3[i][j], confM3[i][j], toplist, charConfM3[i][j] = image_prediction(pred_img, topk = 1, net_num = net_num, labels=name_list, net_type = net_type)
